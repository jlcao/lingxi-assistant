...
# 直接导入 soul 模块（不通过 lingxi 包）
sys.path.insert(0, os.path.dirname(os.path.dirname(os.path.abspath(__file__))))

from soul.soul_parser import SoulParser
from soul.soul_cache import SoulCache
from soul.soul_injector import SoulInjector
...
def test_soul_parser():
    ...
    # 测试身份解析
    assert result["identity"]["name"] == "灵犀", f"身份名称解析失败：{result['identity']['name']}"
    assert result["identity"]["creature"] == "AI Assistant", f"生物类型解析失败：{result['identity']['creature']}"
    assert "Warm" in result["identity"]["vibe"], f"氛围解析失败：{result['identity']['vibe']}"
    assert result["identity"]["emoji"] == "🦄", f"表情符号解析失败：{result['identity']['emoji']}"
    print("✓ 身份解析测试通过")
    ...
    # 测试上下文解析
    assert "current context" in result["context"], f"上下文解析失败：{result['context']}"
    print("✓ 上下文解析测试通过")
...
def test_soul_injector():
    ...
    try:
        ...
        # 测试解析
        data = injector.parse()
        assert data["identity"]["name"] == "灵犀", f"解析失败：{data['identity']['name']}"
        ...
        assert len(injected) == len(messages), f"消息数量改变：{len(injected)} != {len(messages)}"
        ...
        assert "Name: 灵犀" in summary, f"身份摘要不包含名称：{summary}"
        assert "Creature: AI Assistant" in summary, f"身份摘要不包含生物类型：{summary}"
        print("✓ 获取身份摘要测试通过")
        ...
        assert injector.soul_data["identity"]["name"] == "Modified", f"重新加载后数据未更新：{injector.soul_data['identity']['name']}"
        print("✓ 重新加载测试通过")
...
def test_integration():
    ...
    try:
        ...
        injected = injector.inject(messages)
        assert len(injected) == 2, f"消息数量错误：{len(injected)}"
        assert injected[0]["role"] == "system", "系统消息未添加"
        
        summary = injector.get_identity_summary()
        assert "灵犀" in summary, f"身份摘要错误：{summary}"
        ...
        assert injector.parse()["identity"]["name"] == "New", f"重新加载后数据未更新：{injector.parse()['identity']['name']}"
